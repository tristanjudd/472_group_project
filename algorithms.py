...
def alpha_beta_minimax(board: Board, is_max: bool, e: Callable[[Board], int], depth: int, MAX_DEPTH: int, beta: int, alpha: int, is_alpha_beta: bool) -> int:
  if depth == MAX_DEPTH:
    return e(board)
  ...
  for c_move in board.move_candidates(current_player):
    ...
    c_score = alpha_beta_minimax(c_board, not is_max, e, depth + 1, MAX_DEPTH, beta, alpha, is_alpha_beta)
